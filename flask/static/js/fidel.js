const partners = [
    {
        ...
        cart: {
            price: '.cart_amount', photo: '.shopping_cart_area'
        },
        payment: {
            price: '.Checkout_section.mt-60>.container>.checkout_form>.row>.col-lg-6.col-md-6:nth-child(2)>form>.order_table.table-responsive>table>tfoot>tr:nth-child(3)>td>strong',
            photo: '.Checkout_section.mt-60'
        },
        congrats: '.tracking-product.mt-60.mb-60'
    },
    {
        name: 'https://unnostore.com',
        cart: {
            price: '.cart_totals>table>tbody>.order-total>td',
            photo: '.ast-container'
        },
        payment: {
            price: '.shop_table.woocommerce-checkout-review-order-table>tfoot>tr:nth-child(3)>td',
            photo: '.checkout.woocommerce-checkout'
        },
        congrats: '.woocommerce-order'
    },
    {
        name: 'https://www.perfezione-lingerie.com',
        cart: {
            price: '.product__price', photo: '.content'
        },
        payment: {
            price: '.payment-due__price', photo: '.content'
        },
        congrats: ''
    },
    {
        name: 'https://www.nathaliebird.pe',
        cart: {
            price: '.price', photo: '.cart'
        },
        payment: {
            price: '.pplr_subtotal_price', photo: '.content'
        },
        congrats: ''
    },
    {
        name: 'https://hannahs.company.site',
        cart: {
            price: '.ec-cart-item__price-inner', photo: '.ecwid-productBrowser-CartPage'
        },
        payment: {
            price: '.ec-cart-summary__total', photo: '.ec-store__checkout-page'
        },
        congrats: ''
    },
    {
        name: 'https://lannasleepwear.com',
        cart: {
            price: '.cart-collaterals>.cart_totals>table>tbody>tr:nth-child(3)>td', photo: '.shop_table'
        },
        payment: {
            price: '#order_review>table>tfoot>tr:nth-child(3)>td', photo: '.astra-mobile-cart-overlay'
        },
        congrats: ''
    },
    {
        name: 'https://corionica.com',
        cart: {
            price: '.cart-subtotal__price', photo: '#shopify-section-cart-template'
        },
        payment: {
            price: '.payment-due__price skeleton-while-loading--lg', photo: '.wrap'
        },
        congrats: ''
    },
    {
        name: 'https://topdanza.com',
        cart: {
            price: '.cart-collaterals>.cart_totals>table>tbody>tr:nth-child(2)>td',
            photo: 'body>.wrapper>.pt-50.pb-50>.container'
        },
        payment: {
            price: '#order_review>table>tfoot>tr:nth-child(2)>td', photo: '.content-woocommerce>.woocommerce'
        },
        congrats: '.content-woocommerce.pl-30.px-0>.woocommerce>.woocommerce-order'
    },
]

// Sites paths our partners
const cartPartnersPath = ['cart', 'carrito']

// Sites paths our partners
const checkoutPartnersPath = ['checkout', 'checkouts', 'payments', 'finalizar-compra']

// Sites paths our partners
const congratsPartnersPath = ['thank_you', 'orders', 'tracking', 'order-received']

// LOCALSTORE
const base_path = 'fidel_sdk_'
const key_profile = base_path + 'profile'
const key_uuid = base_path + 'uuid_value'
const key_db = base_path + 'db_uuid'
const key_time = base_path + 'time'

//Query params [keys]
const fidel_uuid_key = "fidel_uuid";
const fidel_profile_key = "fidel_profile";

// Global varibles
var global_fidel_uuid = null
var global_fidel_profile = null
var global_fidel_time = null

// Service
// const service = 'http://localhost:8000/sdk/transaction/'
const service = 'https://panel.fidel.pe/sdk/transaction/'
...
function fidel_sdk() {
    ...
    console.log(base_path + "url => ", url)
    partners.forEach(domain => {

        if (domain.name === url.origin) {
            console.log("Domain => ", domain.name)
            let path = url.pathname.toLowerCase()

            // Get uuid
            global_fidel_uuid = url.searchParams.get(fidel_uuid_key)
            global_fidel_profile = url.searchParams.get(fidel_profile_key)
            if (global_fidel_uuid != null) {
                console.log("Fidel SDK Installed")
                let config = {
                    uuid: global_fidel_uuid,
                    domain: domain.name,
                    path: path,
                    profile: global_fidel_profile,
                    item: domain
                };
                start_step(config)
            }

            global_fidel_uuid = localStorage.getItem(key_uuid) || null
            global_fidel_profile = localStorage.getItem(key_profile) || null
            global_fidel_time = localStorage.getItem(key_time) || null // time
            // if (global_fidel_uuid != null && global_fidel_profile != null) {
            if (global_fidel_uuid != null) {
                console.log("Fidel SDK Settings!")
...
function start_step(settings) {
    ...
    var profile_storage = localStorage.getItem(key_profile) || null
    // if (uuid_storage == null && profile_storage == null) {
    if (uuid_storage == null) {
        localStorage.setItem(key_uuid, settings.uuid)
        localStorage.setItem(key_profile, settings.profile)

        sendParams('post', {
            'start_step': true,
            company: settings.uuid,
            ...
            localStorage.setItem(key_db, data.uuid)
...
async function cart_step(settings) {
    console.log("CART", settings)

    const price = getPrice(settings.item.cart.price)
    const form = await getImageBase64(settings.item.cart.photo)

    var db_storage = localStorage.getItem(key_db) || null
    var uuid_storage = localStorage.getItem(key_uuid) || null
    var profile_storage = localStorage.getItem(key_profile) || null
    ...
    // if (db_storage != null && uuid_storage != null && profile_storage != null) {
    if (db_storage != null && uuid_storage != null) {
        sendParams('put', {
            'cart_step': true,
            company: settings.uuid,
            profile: settings.profile,
...
async function payment_step(settings) {
    console.log("PAYMENT", settings)
    const price = getPrice(settings.item.payment.price)
    const form = await getImageBase64(settings.item.payment.photo)
    ...
    if (db_storage != null && uuid_storage != null) {
        sendParams('put', {
            ...
            console.log('Success PAYMENT Update!')
        })
    }
    console.log(localStorage)
}

async function congrats_step(settings) {
    console.log("congrats_step", settings)

    const form = await getImageBase64(settings.item.congrats)
...
async function getImageBase64(query) {
    console.log('start capture!')
    let capture;
    try {
        const canvas = await html2canvas(document.querySelector(query));
        // document.body.appendChild(canvas);
        capture = canvas.toDataURL('image/jpeg').replace('image/jpeg', 'image/octet-stream');
...
function getPrice(query) {
    let price;
    try {
        price = document.querySelector(query).textContent
        console.log(price)
    } catch (e) {
        price = 'Error al optener el precio'
    }
    return price
}
...
(function (global, factory) {
     ...
     */
        ...
        var decode$1 = function (base64) {
            var bufferLength = base64.length * 0.75, len = base64.length, i, p = 0, encoded1, encoded2, encoded3,
                encoded4;
        ...
        var Parser = /** @class */ (function () {
            ...
            Parser.prototype.consumeFunction = function (functionToken) {
                var cssFunction = {
                    name: functionToken.value, values: [], type: 18 /* FUNCTION */
                };
                while (true) {
                    var token = this.consumeToken();
        ...
        var ZERO_LENGTH = {
            type: 17 /* NUMBER_TOKEN */, number: 0, flags: FLAG_INTEGER
        };
        var FIFTY_PERCENT = {
            type: 16 /* PERCENTAGE_TOKEN */, number: 50, flags: FLAG_INTEGER
        };
        var HUNDRED_PERCENT = {
            type: 16 /* PERCENTAGE_TOKEN */, number: 100, flags: FLAG_INTEGER
        };
        ...
        var TURN = 'turn';
        var angle = {
            name: 'angle', parse: function (_context, value) {
        ...
        var color$1 = {
            name: 'color', parse: function (context, value) {
        ...
        var findCorner = function (width, height, x, y, closest) {
            ...
            return corners.reduce(function (stat, corner) {
                ...
                optimumDistance: closest ? Infinity : -Infinity, optimumCorner: null
            }).optimumCorner;
        };
        ...
        var prefixLinearGradient = function (context, tokens) {
            ...
            return {
                angle: angle$1, stops: stops, type: CSSImageType.LINEAR_GRADIENT
            };
        };
        ...
        var webkitGradient = function (context, tokens) {
            ...
            return type === CSSImageType.LINEAR_GRADIENT ? {
                angle: (angle + deg(180)) % deg(360), stops: stops, type: type
        ...
        var image = {
            name: 'image', parse: function (context, value) {
        ...
        var color = {
            name: "color", initialValue: 'transparent', prefix: false, type: 3 /* TYPE_VALUE */, format: 'color'
        };
        ...
        var lineHeight = {
            name: 'line-height', initialValue: 'normal', prefix: false, type: 4 /* TOKEN_VALUE */
        };
        ...
        var marginForSide = function (side) {
            return ({
                name: "margin-" + side, initialValue: '0', prefix: false, type: 4 /* TOKEN_VALUE */
            });
        };
        ...
        var textShadow = {
            ...
            parse: function (context, tokens) {
                ...
                return parseFunctionArgs(tokens).map(function (values) {
                    var shadow = {
                        color: COLORS.TRANSPARENT, offsetX: ZERO_LENGTH, offsetY: ZERO_LENGTH, blur: ZERO_LENGTH
                    };
                    var c = 0;
        ...
        var SUPPORTED_TRANSFORM_FUNCTIONS = {
            matrix: matrix, matrix3d: matrix3d
        };
        ...
        var time = {
            name: 'time', parse: function (_context, value) {
                if (value.type === 15 /* DIMENSION_TOKEN */) {
        ...
        var opacity = {
            name: 'opacity', initialValue: '1', type: 0 /* VALUE */, prefix: false, parse: function (_context, token) {
                if (isNumberToken(token)) {
                    return token.number;
                }
                return 1;
        ...
        var fontSize = {
            name: "font-size", initialValue: '0', prefix: false, type: 3 /* TYPE_VALUE */, format: 'length'
        };
        ...
        var duration = {
            name: 'duration', initialValue: '0s', prefix: false, type: 1 /* LIST */, parse: function (context, tokens) {
        ...
        var quotes = {
            name: 'quotes', initialValue: 'none', prefix: true, type: 1 /* LIST */, parse: function (_context, tokens) {
                if (tokens.length === 0) {
                    return null;
                }
                var first = tokens[0];
     ...
     */
                ...
                declaration.content.forEach(function (token) {
                    if (token.type === 0 /* STRING_TOKEN */) {
                        ...
                        if (token.name === 'attr') {
                            ...
                            var _a = token.values.filter(nonFunctionArgSeparator), counter = _a[0],
                                counterStyle = _a[1];
        ...
        var CanvasRenderer = /** @class */ (function (_super) {
            ...
            CanvasRenderer.prototype.renderStackContent = function (stack) {
                return __awaiter(this, void 0, void 0, function () {
                    var _i, _a, child, _b, _c, child, _d, _e, child, _f, _g, child, _h, _j, child, _k, _l, child, _m,
                        _o,
                        child;
                        ...
                        switch (_b.label) {
                            case 0:
                                ...
                                _loop_1 = function (backgroundImage) {
                                    var image, url, _c, path, x, y, width, height, pattern, _d, path, x, y, width,
                                        height,
                                        _e, lineLength, x0, x1, y0, y1, canvas, ctx, gradient_1, pattern, _f, path,
                                        left,
                                        top_1, width, height, position, x, y, _g, rx, ry, radialGradient_1, midX, midY,
                                        f,
                                        invF;
        ...
        var renderElement = function (element, opts) {
            return __awaiter(void 0, void 0, void 0, function () {
                var ownerDocument, defaultView, resourceOptions, contextOptions, windowOptions, windowBounds, context,
                    foreignObjectRendering, cloneOptions, documentCloner, clonedElement, container, _a, width, height,
                    left,
